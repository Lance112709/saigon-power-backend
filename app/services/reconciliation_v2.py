...
def load_deals(db, provider_group: str) -> dict:
    ...
    if provider_group == "Discount Power/Cirro":
        # the NRG residual report also carries a "Reliant Residual" line for a
        # couple of commercial accounts labelled Reliant in the CRM
        wanted.add("reliant")
        wanted.add("reliant energy")
    ...
    for d in fetch_all(db, "lead_deals",
                       ...
                       "provider_status_date,service_address,service_zip,sales_agent,leads(first_name,last_name,phone)"):
    ...
    for d in fetch_all(db, "crm_deals",
                       ...
                       "provider_status_date,crm_customers(full_name,phone,postal_code)"):
